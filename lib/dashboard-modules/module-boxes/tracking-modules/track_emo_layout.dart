...
class _TrackEmoLayoutState extends State<TrackEmoLayout> {
        ...
        if (poses.isNotEmpty) {
          ...
          for (var pose in poses) {
            ...
            if (detectedEmotionResult != null && detectedEmotionResult.isNotEmpty) {
              ...
              print('Detected Emotion: $detectedEmotionResult'); // Print detected emotion for debugging
            }
          }
        } else {
  ...
  // Custom session and camera controls
  void _endSession() async {
    ...
    try {
      ...
      // After the dialog is dismissed, navigate back to the dashboard
      Navigator.of(context).pop(); // Pop back to the dashboard or previous screen
    ...
    _resetUI(); // Reset the UI state after session ends
  }

  void _toggleCameraPlaying() {
    if (_isCameraPlaying) {
      _pauseLiveFeed(); // Pause the camera feed and stop tracking
    } else {
  ...
  void _startTracking() {
    _startLiveFeed(); // Start the camera feed
